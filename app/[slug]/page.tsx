...
export async function generateMetadata({
  ...
  const title = seoContent
    ? `${seoContent.title} | Suppl.me`
    : `${route.title} - Suppl.me`;
  const description = seoContent
    ? seoContent.description
    : route.description ||
      `Evidence-based information about ${route.title} supplements: benefits, dosing, safety, and retailer comparison.`;
  const keywords = seoContent
    ? seoContent.keywords.join(", ")
    : `${
        route.title
      }, ${route.title.toLowerCase()} supplements, ${route.title.toLowerCase()} benefits, ${route.title.toLowerCase()} dosage, supplement research`;
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: `${baseUrl}/images/supplements/${slug}.webp`,
          width: 1200,
          height: 630,
          alt: `${
            seoContent ? seoContent.name : route.title
    ...
    twitter: {
      ...
      title: seoContent ? seoContent.title : route.title,
      description,
      images: [`${baseUrl}/images/supplements/${slug}.webp`],
    },
    alternates: {
      canonical: fullUrl,
    },
...
export default async function SupplementPage({ params }: PageProps) {
  ...
  if (!Component) {
    console.error(
      `Component ${route.componentName} not found in COMPONENT_MAP`
    );
    console.error(
