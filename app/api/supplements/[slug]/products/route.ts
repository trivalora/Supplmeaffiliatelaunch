...
export async function GET(
  ...
  try {
    ...
    let products = (data || []).map((product: any) => {
      ...
      const bestPrice = filteredPrices.length > 0 
    ...
    if (sortField === 'price') {
      products.sort((a, b) => {
        const aPrice = a.best_total_price || Infinity;
        const bPrice = b.best_total_price || Infinity;
    ...
    console.error('Unexpected error in /api/supplements/[slug]/products:', error);
    return NextResponse.json(
