...
supplementDescriptions.forEach((desc, i) => {
  ...
  const status = length >= 150 && length <= 160 ? '✅' : length > 160 ? '⚠️' : '❌';
  console.log(`  ${status} ${length} chars - ${desc.substring(0, 60)}...`);
  
  if (length < 150) issues.push(`Supplement page ${i + 1}: Too short (${length} chars)`);
  if (length > 160) issues.push(`Supplement page ${i + 1}: Too long (${length} chars)`);
...
const staticPages = [
  { name: 'Home', desc: 'Comprehensive evidence-based supplement information with clinical research, meta-analysis reviews, dosing recommendations, safety profiles, and multi-retailer price comparison. Compare prices from iHerb, Amazon, Vitacost, and more.' },
  { name: 'About', desc: 'Meet the team behind the evidence-based supplement platform. Learn about our mission to provide transparent, science-backed supplement recommendations and price comparisons.' },
  { name: 'Contact', desc: 'Get in touch with the suppl.me team.' },
  { name: 'Partner', desc: 'Partner with suppl.me to provide evidence-based supplement information to your audience.' },
  { name: 'Methodology', desc: 'Learn about our rigorous research methodology for evaluating supplements. We combine academic research precision with modern technology to deliver reliable, evidence-based supplement information.' },
  { name: 'Privacy Policy', desc: 'Our privacy policy and data protection practices.' },
  { name: 'Terms of Service', desc: 'Terms and conditions for using suppl.me.' },
  { name: 'Cookie Policy', desc: 'Information about cookies and tracking technologies we use.' },
  { name: 'Legal Notice', desc: 'Legal notice and disclaimer information.' },
  { name: 'Glossary Index', desc: 'Scientific and medical terms related to supplements, nutrition, and health.' }
];

staticPages.forEach(page => {
  const length = page.desc.length;
  const status = length >= 150 && length <= 160 ? '✅' : length > 160 ? '⚠️' : length < 100 ? '❌' : '⚠️';
  console.log(`  ${status} ${page.name.padEnd(20)} ${length} chars - ${page.desc.substring(0, 50)}...`);
  ...
  if (descriptions.has(page.desc)) {
    issues.push(`${page.name}: Duplicate description with ${descriptions.get(page.desc)}`);
  }
...
if (error) {
  ...
  if (duplicates.size > 0) {
    ...
    duplicates.forEach((slugs, desc) => {
      console.log(`     - "${desc.substring(0, 50)}..." appears in: ${slugs.join(', ')}`);
    });
  } else {
...
console.log('  Format: "Compare prices and view supplement facts for {brand} {name}..."');
