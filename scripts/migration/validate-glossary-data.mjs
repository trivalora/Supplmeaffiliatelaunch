...
async function validateData() {
  try {
    ...
    if (missing.length > 0) {
      ...
      missing.forEach((t) =>
        console.log(`      - ${t.slug || "NO SLUG"}: ${t.term || "NO TERM"}`),
      );
      return false;
    } else {
      console.log(
        `   ✅ All ${allTerms.length} terms have required fields (slug, term, definition)`,
      );
    }
    ...
    if (duplicates.length > 0) {
      console.log(
        `   ❌ Duplicate slugs found: ${[...new Set(duplicates)].join(", ")}`,
      );
      return false;
    } else {
    ...
    console.log(
      `   ✅ ${termsWithRelations?.length || 0} terms have related terms`,
    );
    ...
    try {
      const response = await fetch(
        "http://localhost:3000/api/glossary?limit=1",
      );
      if (response.ok) {
        const data = await response.json();
        console.log(
          `   ✅ API endpoint working (${data.total} terms available)`,
        );
      } else {
        console.log(
          `   ⚠️  API returned ${response.status} (is dev server running?)`,
        );
      }
    } catch (apiError) {
    ...
    console.log(
      "  2. View single term: curl http://localhost:3000/api/glossary/rct",
    );
    console.log(
      '  3. Search: curl "http://localhost:3000/api/glossary?search=clinical"',
    );
