...
const vitacost = data.filter(p => p.product_image_url?.includes('vitacost.com'));
const iherb = data.filter(p => p.product_image_url?.includes('images-iherb.com'));
const nullUrls = data.filter(p => !p.product_image_url || p.product_image_url.length === 0);
...
console.log(`✅ Vitacost URLs: ${vitacost.length} → remaining-vitacost-urls.txt`);
...
const nullOutput = nullUrls.map(p => 
  `${p.id}\t${p.brand || 'N/A'}\t${p.product_name}`
).join('\n');
writeFileSync('products-without-images.txt', 
  'Product ID\tBrand\tProduct Name\n' + nullOutput
);
console.log(`❌ Products without images: ${nullUrls.length} → products-without-images.txt`);

// Export detailed CSV for all external URLs
const csvHeader = 'product_id,brand,product_name,image_url,source\n';
const csvRows = [
  ...vitacost.map(p => `${p.id},"${p.brand || ''}","${p.product_name}","${p.product_image_url}",vitacost`),
  ...iherb.map(p => `${p.id},"${p.brand || ''}","${p.product_name}","${p.product_image_url}",iherb`)
].join('\n');
writeFileSync('external-images-detailed.csv', csvHeader + csvRows);
console.log(`📋 Detailed CSV: ${vitacost.length + iherb.length} → external-images-detailed.csv`);
