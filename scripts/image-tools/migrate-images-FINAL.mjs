...
async function migrateImages() {
  console.log("🚀 Starting Image Migration (FIXED)...\n");
  ...
  for (const record of records) {
    ...
    // Fix: Vitacost files don't have vitacost_ prefix in actual filesystem
  ...
  for (const filename of mapping.values()) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      missingCount++;
      if (missingFiles.length < 10) missingFiles.push(filename);
    }
  }
  ...
  if (missingCount > 0 && missingFiles.length > 0) {
  ...
  for (const product of products) {
    ...
    // Skip if already migrated to local
    if (oldUrl.startsWith("/images/")) {
      alreadyLocal.push({ id: product.id, url: oldUrl });
      continue;
    }
  ...
  if (updates.length === 0) {
    console.log("✅ No updates needed! All images are already migrated.\n");
    return;
  }
  ...
  const summary = {
    ...
    database_updates: {
      updated,
      failed,
      already_local: alreadyLocal.length,
