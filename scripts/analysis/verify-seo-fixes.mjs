...
descriptions.forEach((desc, i) => {
  ...
  if (length < 130 || length > 160) {
    supplementIssues++;
    console.log(`  ${status} Supplement ${i + 1}: ${length} chars - ${desc.substring(0, 50)}...`);
  }
});

if (supplementIssues === 0) {
  console.log(`  ✅ All ${descriptions.length} supplement descriptions optimized (130-160 chars)\n`);
} else {
  console.log(`  ⚠️  ${supplementIssues} supplement descriptions need adjustment\n`);
...
if (staticIssues === 0) {
  console.log(`  ✅ All ${staticPages.length} static pages optimized (130-165 chars)\n`);
} else {
...
const sectionImage = readFileSync('src/components/images/SectionImage.tsx', 'utf-8');
const productImage = readFileSync('src/components/images/ProductImage.tsx', 'utf-8');

const sectionSizes = sectionImage.includes('min-width: 640px') && 
                     sectionImage.includes('min-width: 768px') &&
                     sectionImage.includes('min-width: 1280px');

const productSizes = productImage.includes('min-width: 640px') && 
