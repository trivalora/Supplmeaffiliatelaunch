...
- ✅ **Scalability Plan**: `docs/SCALABILITY_IMPLEMENTATION_PLAN.md` created (Nov 26, 2025)

**MAJOR SCALABILITY UPDATE (Nov 26, 2025)**:
🚀 **Database Migration Plan Ready**: Comprehensive 6-week plan to migrate from file-based (JSON) to database-driven architecture (Supabase/PostgreSQL)
- **Timeline**: 6 weeks, 60 hours total effort
- **Scope**: Skip Phase 2 (intermediate fixes), implement Phase 3 (enterprise database) directly
- **Benefits**: Support 100+ supplements, real-time updates, advanced search, user features
- **Status**: ✅ Detailed plan documented, ready for implementation
- **See**: `docs/SCALABILITY_IMPLEMENTATION_PLAN.md` for complete details

**Priority Improvements (Updated Nov 26, 2025):**

**🔴 CRITICAL (Do This Week - 5 hours total):**
1. ~~Implement dynamic component loading~~ **SUPERSEDED** - Will be handled by database migration
...
3. Remove 'v2' suffix from route keys in routes.config.ts (2h)
   - 17 route keys affected (ashwagandhav2 → ashwagandha)
   - Legacy naming causes developer confusion

**🟠 HIGH PRIORITY (Next 6 Weeks - 60 hours total):**
4. **Database Migration** (see SCALABILITY_IMPLEMENTATION_PLAN.md)
   - Week 1: Supabase setup, schema design (8h)
   - Week 2: Database implementation, data migration (12h)
   - Week 3: API development (16h)
   - Week 4: Frontend integration (12h)
   - Week 5: Testing & validation (8h)
   - Week 6: Deployment & monitoring (4h)

**⚠️ MEDIUM PRIORITY (After Migration):**
5. Standardize styling approach (2h - documentation)
6. Create StaticPageTemplate for consistency (4h)
7. Admin panel for product management (future)
...
- `/api/events.ts` - Analytics event tracking
- `/api/partner-lead.ts` - Partner lead submission (optional PostgreSQL)
- `/api/subscribe.ts` - Email subscription (optional PostgreSQL)
- **Database Support**: Optional PostgreSQL via `api/_lib/db.ts` (currently optional, will become primary in Phase 3)

**Future (Phase 3 - Database Migration)**: 
- `/api/supplements` - Supplement listing and details
- `/api/products` - Product data from PostgreSQL/Supabase
- `/api/retailers` - Retailer information
- `/api/glossary` - Glossary terms
- See `docs/SCALABILITY_IMPLEMENTATION_PLAN.md` for complete API architecture
...
- Current: 198 terms processed on every content render
...
### Database Migration Strategy (Phase 3)

**See**: `docs/SCALABILITY_IMPLEMENTATION_PLAN.md` for complete details

**Why migrate to database?**
- ✅ Support 100+ supplements without performance degradation
- ✅ Real-time price updates (no rebuild required)
- ✅ Advanced search & filtering capabilities
- ✅ User features (favorites, price alerts)
- ✅ Admin panel for product management
- ✅ Better developer experience (API-driven)

**Technology Stack**:
- **Database**: Supabase (PostgreSQL with built-in REST API)
- **Caching**: Next.js ISR + Edge Cache
- **Search**: Future integration with Algolia/Typesense
- **Images**: Future CDN (Cloudinary/Imgix)

**Timeline**: 6 weeks, 60 hours total effort
**Phases**:
1. Week 1: Database setup & schema design (8h)
2. Week 2: Data migration & validation (12h)
3. Week 3: API development (16h)
4. Week 4: Frontend integration (12h)
5. Week 5: Testing & optimization (8h)
6. Week 6: Deployment & monitoring (4h)

**Key Benefits**:
- Maintain current SEO performance (ISR)
- Zero downtime migration (feature flags)
- Rollback plan (keep JSON files as backup)
- Cost-efficient (Supabase free tier supports 50+ supplements)
