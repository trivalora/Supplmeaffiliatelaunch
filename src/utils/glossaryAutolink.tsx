...
export function autolinkGlossaryTerms(
  ...
  // If no navigation handler, return plain text
  if (!onNavigate) {
    return [text];
  }
...
export function useAutolinkGlossaryTerms(
  ...
  return useMemo(() => {
    return autolinkGlossaryTerms(text, onNavigate, currentPage);
  }, [text, onNavigate, currentPage]);
}

/**
 * MEMOIZED VERSION: Hook to process text content with line breaks and caching
 * Use this in components to prevent re-processing on every render
...
export function useAutolinkGlossaryContent(
  ...
  return useMemo(() => {
    return autolinkGlossaryContent(content, onNavigate, currentPage);
  }, [content, onNavigate, currentPage]);
}