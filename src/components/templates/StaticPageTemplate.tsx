...
export function ContentSection({
  ...
  const containerMaxWidth = maxWidth === 'narrow' ? 'max-w-[800px]' : 'max-w-7xl';
...
export function StaticPageTemplate({
  ...
  const heroBgClass = 
    heroBackground === 'secondary' ? 'bg-secondary' :
    heroBackground === 'tertiary' ? 'bg-tertiary' :
    'bg-primary';
  
  const heroTextColor = heroBackground === 'primary' ? 'text-tertiary' : 'text-primary';
  const heroSubtitleColor = heroBackground === 'primary' ? 'text-secondary' : 'text-foreground/80';
  ...
  return (
    <>
      ...
      {/* Main Container */}
      <div className={`bg-tertiary flex flex-col w-full min-h-screen ${className}`} data-page-content>
        
        {/* Top Anchor (for legal pages with "back to top" links) */}
        {showTopAnchor && (
          <div id="top" className="absolute" style={{ top: 'var(--header-height)' }} />
        )}
        ...
        <div id="hero" className={heroBgClass}>
          <div className="flex-1 flex items-center justify-center px-6 py-16 md:py-24">
            <div className="max-w-[800px] text-center">
              ...
              {/* Hero Title */}
              <h1 className={`font-['Lora',serif] text-[32px] md:text-[48px] leading-tight mb-6 ${heroTextColor}`}>
                {heroTitle}
              </h1>
              
              {/* Hero Subtitle (optional) */}
              {heroSubtitle && (
                <p className={`text-[18px] md:text-[20px] leading-[32px] ${heroSubtitleColor}`}>
                  {heroSubtitle}
                </p>
              )}
            </div>
...
export function CardContent({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <div className={`bg-card rounded-[14px] border border-secondary p-6 md:p-10 ${className}`}>
      {children}
    </div>
  );
}
...
export function InfoBlock({ icon: Icon, title, description, children }: InfoBlockProps) {
  return (
