...
export function KnowledgebasePage({ onNavigate }: KnowledgebasePageProps) {
  ...
  const visibleRoutes = KNOWLEDGEBASE_ROUTES.filter((route) => route.showInNav);
  ...
  Object.keys(routesBySubcategory).forEach((key) => {
    routesBySubcategory[key as SubcategoryType].sort((a, b) =>
  ...
  const sortedSubcategories = subcategoryOrder.filter(
    (cat) => routesBySubcategory[cat]
  );

  // Filter for search
  const filteredItems = visibleRoutes.filter(
    (route) =>
          ...
          <div data-layout-container>
            <div className="max-w-4xl mx-auto text-center">
              <h1 className="text-primary-foreground mb-4">
                Supplement Knowledgebase
              </h1>
              <p className="text-secondary text-lg md:text-xl mb-8">
                Evidence-based information on supplements, backed by scientific
                research
              </p>
