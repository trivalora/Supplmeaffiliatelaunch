...
export function SearchResults({ query, onNavigate, context: _context = 'header' }: SearchResultsProps) {
  ...
  const filteredResults = allSearchableRoutes.filter((route: any) => {
  ...
  const knowledgebaseResults = filteredResults.filter((r: any) => r.category === 'knowledgebase' || !r.category);
  const glossaryResults = filteredResults.filter((r: any) => r.category === 'glossary');
